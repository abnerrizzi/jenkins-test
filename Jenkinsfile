                ...
                */
                ...
                if [ "$value" -gt 0 ] && [ "$value" -lt 65536 ]; then
                    ...
                    if [ "$params.DB_TYPE" == "mysql" ]; then
                        ...
                        docker run -it -d --rm --name ${containerName} -e ORACLE_USER=developer -e ORACLE_PASSWORD=$params.DB_PASS -e ORACLE_DB=DEVAPP -p $params.DB_PORT:1521 --name $containerName -v $localsqldir:/docker-entrypoint-initdb.d oracle
                    fi
